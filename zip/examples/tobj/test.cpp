...
void replace_all(std::string & str, char * oldValue, char * newValue)
{
	...
	while (true)
	{
		...
		if (pos != (std::string::npos))
		{
			...
			pos += 2;//×¢ÒâÊÇ¼Ó2£¬ÎªÁËÌøµ½ÏÂÒ»¸ö·´Ð±¸Ü
		}
		else
			break;
	}
...
int mainbe(int argc, char **argv)
{
	...
	std::string myexe = argv[i++];
	while (i < argc && myexe.substr(myexe.length() - 4, 4) != ".exe")
	{
		myexe += " ";
		myexe += argv[i++];
	}
	...
	std::string strZip = argv[i++];
	while (i < argc && strZip.substr(strZip.length() - 4, 4) != ".zip")
	{
		strZip += " ";
		strZip += argv[i++];
	}
	//cout << ("strZip") << endl; 
	std::cout << strZip << endl;


	std::string des = argv[i++];
	while (i < argc && des.substr(des.length() - 3, 3) != ".db")
	{
		des += " ";
		des += argv[i++];
	}
	...
	while (i < argc && src.substr(src.length() - 3, 3) != ".db")
	{
		src += " ";
		src += argv[i++];
	}
	...
	std::string pw = argv[i];
	
	TCHAR zipT[MAX_PATH] = _T("\0");
	TCHAR desT[MAX_PATH] = _T("\0");
	TCHAR srcT[MAX_PATH] = _T("\0");
#ifdef UNICODE
	_stprintf_s(zipT, MAX_PATH, _T("%S"), strZip.c_str());//%S¿í×Ö·û
	_stprintf_s(desT, MAX_PATH, _T("%S"), des.c_str());//%S¿í×Ö·û
	_stprintf_s(srcT, MAX_PATH, _T("%S"), src.c_str());//%S¿í×Ö·û
#else
	_stprintf_s(zipT, MAX_PATH, _T("%s"), strZip.c_str());//%sµ¥×Ö·û
	_stprintf_s(desT, MAX_PATH, _T("%s"), des.c_str());//%sµ¥×Ö·û
	_stprintf_s(srcT, MAX_PATH, _T("%s"), src.c_str());//%sµ¥×Ö·û
#endif

	HZIP hz = CreateZip(zipT, pw.c_str());
	//HZIP hz = CreateZip(zipT, "39c099e4f77f9374d5bc2ea781e42f26");
	ZipAdd(hz, desT, srcT);
	CloseZip(hz);

	string newStrZip = strZip;
	replace_all(newStrZip, ".zip", ".be");
	rename(strZip.c_str(), newStrZip.c_str());
...
int main(int argc, char **argv)
{
	...
	std::string des = argv[i++];
	while (i < argc && des.substr(des.length() - 3, 3) != ".be")
	{
		des += " ";
		des += argv[i++];
	}
	...
	std::string src = argv[i++];
	while (i < argc && src.substr(src.length() - 3, 3) != ".be")
	{
		src += " ";
		src += argv[i++];
	}
	...
	//HZIP hz = CreateZip(zipT, 0);
	HZIP hz = CreateZip(zipT, "39c099e4f77f9374d5bc2ea781e42f26");
	ZipAdd(hz, desT, srcT);
	CloseZip(hz);
	
	string newStrZip = strZip;
	replace_all(newStrZip,"_bj.zip",(".bj"));
	rename(strZip.c_str(), newStrZip.c_str());
...
{ HZIP hz; HANDLE hf; DWORD writ; ZRESULT zr; ZIPENTRY ze; TCHAR m[1024];
  ...
  SaveResource(MAKEINTRESOURCE(1),_T("\\z\\zxtra.zip"));
  ...
  hz=CreateZip(_T("\\z\\zmpty.zip"),0); if (hz==0) msg(_T("* Failed to create empty.zip"));
  zr=CloseZip(hz); if (zr!=ZR_OK) msg(_T("* Failed to close empty.zip"));
  if (p_abort) return 1;
  hz = OpenZip(_T("\\z\\zmpty.zip"), 0); if (hz == 0) msg(_T("* Failed to open empty.zip"));
  ...
  // fixed bug: when adding multiple large files only the first file greater than ~64K is added successfully. Each successive file will only be added if it is less than ~64k. After more testing the exact byte size varies slightly. (might depend on the compression ratio). For example a text file with ~64K of the letter 'a' will fail at exactly 65,276Bytes (File size not Size on Disk)However if the file is mixed letters 'asdjkl' then it will fail consistently at 65,483Bytes.
  // REPRO STEPS: create two text files that are 67K. (To mimick my exact test just use the letter 'a'). Only the first one will be added. When you attempt to add another file that is > ~64K zipAdd Returns ERROR 0x00000400 0xZR_WRITE "a general error writing to the file". Again there is no problem adding additional files if they are <~64K.
  // ERROR ANALYSIS: This is caused by deflate(TXtate &state) setting state.err to "insufficient lookahead"
  ...
  if (p_abort) return 1;
  //
  { char c[67*1024]; memset(c,'a',67*1024);
    hf=CreateFile(_T("\\z\\sizes2-67k1.dat"),GENERIC_WRITE,0,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0); WriteFile(hf,c,71*1024,&writ,0); CloseHandle(hf);
    hf=CreateFile(_T("\\z\\sizes2-67k2.dat"),GENERIC_WRITE,0,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0); WriteFile(hf,c,71*1024,&writ,0); CloseHandle(hf);
    hz=CreateZip(_T("\\z\\sizes2.zip"),0); if (hz==0) msg(_T("* Failed to create sizes2.zip"));
    zr=ZipAdd(hz,_T("sizes2-67k1.out.dat"),_T("\\z\\sizes2-67k1.dat")); if (zr!=ZR_OK) msg(_T("* Failed to add 67k1"));
    zr=ZipAdd(hz,_T("sizes2-67k2.out.dat"),_T("\\z\\sizes2-67k2.dat")); if (zr!=ZR_OK) msg(_T("* Failed to add 67k2"));
    zr=CloseZip(hz); if (zr!=ZR_OK) msg(_T("* Failed to close sizes.zip"));
    if (p_abort) return 1;
    hz=OpenZip(_T("\\z\\sizes2.zip"),0); if (hz==0) msg(_T("* Failed to open sizes2.zip"));
    zr=UnzipItem(hz,0,_T("\\z\\sizes2-67k1.out.dat")); if (zr!=ZR_OK || !fsame(_T("\\z\\sizes2-67k1.dat"),_T("\\z\\sizes2-67k1.out.dat"))) msg(_T("* Failed to unzip all of 67k1"));
    zr=UnzipItem(hz,1,_T("\\z\\sizes2-67k2.out.dat")); if (zr!=ZR_OK || !fsame(_T("\\z\\sizes2-67k2.dat"),_T("\\z\\sizes2-67k1.out.dat"))) msg(_T("* Failed to unzip all of 67k2"));
    zr=CloseZip(hz); if (zr!=ZR_OK) msg(_T("* Failed to close sizes2.zip"));
    if (p_abort) return 1;
  }
  ...
  if (GetFileAttributes(_T("\\z\\zxtra.zip"))!=0xFFFFFFFF)
  { hz=OpenZip(_T("\\z\\zxtra.zip"),0); if (hz==0) msg(_T("extra.zip is absent; can't do this test"));
    zr=GetZipItem(hz,0,&ze); if (zr!=ZR_OK) msg(_T("* Failed to GetItem(extra.txt)"));
    zr=UnzipItem(hz,0,_T("\\z\\zxtra.txt")); if (zr!=ZR_OK) msg(_T("* Failed to UnzipItem(extra.txt)"));
    zr=CloseZip(hz); if (zr!=ZR_OK) msg(_T("* Failed to close extra.zip"));
    if (p_abort) return 1;
    hf=CreateFile(_T("\\z\\zxtra.txt"),GENERIC_READ,FILE_SHARE_READ,0,OPEN_EXISTING,0,0);
  ...
  { TCHAR *c=_T("Hello there this will have to be a very long file if I want to see what I want to see the quick brown fox jumped over the lazy dog and a merry time was had by all at the party few things are more distressing to a well-regulated mind than to see a boy who ought to know better disporting himself at improper moments\r\n");
    hf=CreateFile(_T("\\z\\zxtrapp.txt"),GENERIC_WRITE,0,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0); WriteFile(hf,c,(DWORD)(_tcslen(c)*sizeof(TCHAR)),&writ,0); CloseHandle(hf);
    hz=CreateZip(_T("\\z\\zxtrapp.zip"),"password"); if (hz==0) msg(_T("* Failed to create extrapp.zip"));
    zr=ZipAdd(hz,_T("extrapp.txt"),_T("\\z\\zxtrapp.txt")); if (zr!=ZR_OK) msg(_T("* Failed to add extrapp.txt"));
    zr=CloseZip(hz); if (zr!=ZR_OK) msg(_T("* Failed to close extrapp.zip"));
    if (p_abort) return 1;
    hz=OpenZip(_T("\\z\\zxtrapp.zip"),"password"); if (hz==0) msg(_T("* Failed to open extrapp.zip"));
    zr=GetZipItem(hz,0,&ze); if (zr!=ZR_OK) msg(_T("* Failed to GetItem(extrapp.txt)"));
    zr=UnzipItem(hz,0,_T("\\z\\zxtrapp.out.txt")); if (zr!=ZR_OK) msg(_T("* Failed to unzip extrapp.txt"));
    zr=CloseZip(hz); if (zr!=ZR_OK) msg(_T("* Failed to close extrapp.zip"));
    if (!fsame(_T("\\z\\zxtrapp.txt"),_T("\\z\\zxtrapp.out.txt"))) msg(_T("* Failed to unzip all of extrapp.txt"));
    if (p_abort) return 1;
  }
...
if (any_errors) { msg(_T("Finished")); return 1; }
else { msg(_T("Finished. No errors.")); return 0; }
